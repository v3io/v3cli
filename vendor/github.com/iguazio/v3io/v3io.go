...
func (v3 V3iow) ListBucket(path string) (ListBucketResp,error) {
	...
	if v3.Url == "" {
		...
		res, err := http.Get(fullpath)
		if err != nil {
			return lb, fmt.Errorf("V3iow:ListBucket - failed req %s (%v)", fullpath,err)
		}
		defer res.Body.Close()
		v3.debug("Stat %s\n", res.Status)
		...
		htmlData, err = ioutil.ReadAll(res.Body)
		if err != nil {
			return lb, fmt.Errorf("V3iow:ListBucket - failed to read body of %s (%v)", fullpath,err)
		}
		v3.debug("Resp: %s\n",htmlData)
	}
...
func (v3 V3iow) Get(path string) ([]byte,error) {
	...
	res, err := http.Get(fullpath)
	if err != nil {
		return Data, fmt.Errorf("V3iow:Get - failed req %s (%v)", fullpath,err)
	}
	defer res.Body.Close()
	v3.debug("Stat %s\n", res.Status)
	if res.StatusCode != 200 {
		return Data, fmt.Errorf("V3iow:Get - Error status when accessing %s (%v)", fullpath,res.Status)
	}
	Data, err = ioutil.ReadAll(res.Body)
	if err != nil {
		return Data, fmt.Errorf("V3iow:Get - failed to read body of %s (%v)", fullpath,err)
	}
	v3.debug("Resp: %s\n",Data)
	return Data,nil
}
...
func (v3 V3iow) GetRecords(path string,offset []byte, maxrec,startseq int ) (GetRecordsResp,error) {
	// create GetItem Body
...
func (v3 V3iow) SeekShard(path string, seek string, from int) ([]byte,error) {
	...
	type respstruct struct { Location []byte }
	var resp respstruct
	err = json.Unmarshal(body, &resp)
	if err != nil {
		return nil, fmt.Errorf("V3iow:SeekShard - failed to Unmarshal resp %v (%v)", body,err)
	}
